...
registration_window.config(bg = 'lightpink')

firebase = firebase.FirebaseApplication("https://loginsignin188-default-rtdb.firebaseio.com/", None)
...
def login(): 
    ...
    hexd_password = encrypted_password.hexdigest()
    get_password = firebase.get('/', username)
    if (get_password != None):
        if (get_password == hexd_password):
            messagebox.showinfo('Message', 'Successfully Logged In!')
        else:
            messagebox.showinfo('Message', 'Please Check Your Password.')
    else:
        messagebox.showinfo('Message', 'User Not Registered! \n Get Yourself Registered First To Log In')
...
def register(): 
    ...
    password_result = hashlib.md5(password.encode())
    password_hexd = password_result.hexdigest()
    print(password_hexd)
    firebase.put('https://loginsignin188-default-rtdb.firebaseio.com/', username, password_hexd)
